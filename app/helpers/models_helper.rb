module ModelsHelper
  ...
  def status_badges(model)
    ...
    badges << problem_icon_tag(problems_including_files.visible(problem_settings)) if policy(Problem).show?
    content_tag :span, safe_join(badges, " "), class: "status-badges"
  end

  def problems_including_files(model)
    policy_scope(Problem).where(problematic: model_files + [self])
  end
