...
class ModelsController < ApplicationController
  ...
  def create
    authorize :model
    library = Library.from_param(params[:library])
    uploads = begin
  ...
  def get_model
    @model = Model.includes(:model_files, :creator, :preview_file, :library, :tags, :taggings, :links, :caber_relations).from_param(params[:id])
    authorize @model
    @title = @model.name
  end
  ...
  def set_returnable
    ...
    @new_collection = Collection.from_param(params[:new_collection]) if params[:new_collection]
    @new_creator = Creator.from_param(params[:new_creator]) if params[:new_creator]
    if @model
