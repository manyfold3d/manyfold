module Filterable
  ...
  def filter_by_library(models, library)
    library ? models.where(library: Library.from_param(library)) : models
  end
  ...
  def filter_by_collection(models, collection)
    ...
    when ""
      ...
      @collection = Collection.from_param(collection)
  ...
  def filter_by_creator(models, creator)
    ...
    when ""
      ...
      @creator = Creator.from_param(creator)
  ...
  def filter_by_missing_tag(models, missingtag, library)
    ...
    if missingtag.presence || (missingtag && library)
      tag_regex_build = []
      regexes = ((missingtag != "") ? [missingtag] : Library.from_param(library).tag_regex)
