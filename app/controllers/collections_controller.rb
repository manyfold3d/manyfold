class CollectionsController < ApplicationController
  ...
  def index
    ...
    @unassigned_count = policy_scope(Model).where(deprecated_collection: nil).count
  ...
  def show
    respond_to do |format|
      format.html do
        @models = policy_scope(Model).where(deprecated_collection: @collection)
        prepare_model_list
  ...
  def collection_params
    if is_api_request?
      ...
      ManyfoldApi::V0::CollectionDeserializer.new(object: params[:json], user: current_user, record: @collection).deserialize
    else
      Form::CollectionDeserializer.new(params: params, user: current_user, record: @collection).deserialize
    end
  end
end
