...
class UploadsController < ApplicationController
  ...
  def upload_zip
    ...
    self.save_files(params[:upload], File.join(library.path, ""))

    if (params[:post][:scan_after_upload] == "1") then
  ...
  def save_files(upload, library_path)
    upload['datafiles'].select {|datafile| datafile != '' }.each { |datafile|
      ...
      if !Dir.exist?(dest_folder_name) then
        self.unzip(dest_folder_name, datafile)
      end
    }
  end
