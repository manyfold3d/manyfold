...
class Components::SourcesCard < Components::Base
  ...
  def render?
    @model.relationships.where(predicate: "adapted_from").any?
  end

  def before_template
    @sources = @model.relationships.where(predicate: "adapted_from").map { it.objekt }
  end

  def view_template
    Card title: t("components.sources_card.title"), variant: :secondary do
      div class: "card-text" do
        @sources.each do |source|
          ModelCard(model: source)
        end
      end
    end
  end
