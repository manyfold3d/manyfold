...
class Components::RemixesCard < Components::Base
  ...
  def render?
    @model.reverse_relationships.where(predicate: "adapted_from").any?
  end

  def before_template
    @remixes = @model.reverse_relationships.where(predicate: "adapted_from").map { it.subject }
  end

  def view_template
    Card title: t("components.remixes_card.title"), variant: :secondary do
      div class: "card-text" do
        @remixes.each do |remix|
          ModelCard(model: remix)
        end
      end
    end
  end
