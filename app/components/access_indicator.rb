class Components::AccessIndicator < Components::Base
  # @param object[CaberSubject] The object to show the access details for; must include the CaberSubject concern
  # @param icon[boolean] Whether or not to show an indicator icon
  # @param text[boolean] Whether or not to show a indicator text
  ...
  def render?
    SiteSettings.multiuser_enabled? && !@object.remote? && (current_user&.is_moderator? || current_user&.owns?(@object))
  end
